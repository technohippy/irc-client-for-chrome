...
IRC.Server.prototype.connect = function() {
  this.tcpClient = new TcpClient(this.host, parseInt(this.port, 10), this.encoding);
  this.tcpClient.connect(function() {
    this.tcpClient.addResponseListener(function(response) {
      ...
      for (var i = 0; i < replies.length; i++) {
        ...
        var channel, channelName, j;
        if (message.command === 'PRIVMSG') {
          // TODO
          message.interprete();
          if (message.isToChannel) {
            // public message among the channel
            channel = this.getChannel(message.channelName);
            channel.messages.push(message);
          }
          else {
            // private message from a user
            channel = this.getOrCreateChannel(message.sender);
          ...
          if (channel) {
            if (0 < channel.addMember(message.sender).length) {
              for (j = 0; j < this.memberListeners.length; j++) {
          ...
          if (message.params[0].match(/^:Closing Link:/)) {
            this.closed = true;
            for (j = 0; j < this.channelListeners.length; j++) {
          ...
          channelName = message.params[message.params.length - 2];
        ...
        for (j = 0; j < this.replyListeners.length; j++) {
