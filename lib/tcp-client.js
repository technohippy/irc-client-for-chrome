...
(function(exports) {
   ...
   */
  function TcpClient(host, port, encoding) {
    this.host = host;
    this.port = port;
  ...
  TcpClient.prototype.sendMessage = function(msg, callback) {
    ...
    this._stringToArrayBuffer(msg + '\n', function(arrayBuffer) {
      socket.write(this.socketId, arrayBuffer,
this._onWriteComplete.bind(this));
   ...
   *
   * @param {Function} callback The function to call when a message has
   * arrived
   */
  ...
  TcpClient.prototype._onCreate = function(createInfo) {
    ...
    if (this.socketId > 0) {
      socket.connect(this.socketId, this.host, this.port,
this._onConnectComplete.bind(this));
   ...
   *
   * @private
   * @param {Number} resultCode Indicates whether the connection was
   * successful
   */
  ...
  TcpClient.prototype._arrayBufferToString = function(buf, callback) {
    ...
    */
    var string = TextDecoder(this.encoding).decode(new DataView(buf));
    callback(string);
  };
